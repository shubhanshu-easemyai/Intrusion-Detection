...
class LogicHandler:
    ...
    def post_process(
        ...
        **kwargs,
        ...
        #         "priority": "medium",
        #         "alert_text": "intrusion detection in region {}".format(
        #             glb.sources_list[index]["roi"]["roi_name"]
        #         ),
        #         "metadata": medias,
        #     }
    ...
    def process_data(self, data, **kwargs):
        try:
            ...
            # logger.debug(glb.loaded_camera_ids[source_details["source_id"]])
            # logger.debug(glb.loaded_camera_ids[source_details["source_id"]]["extra"][0]["roi"]["roi_type"])
            ...
            for detected_object in copy.deepcopy(data["detections"]):
                if detected_object["name"] == glb.object_class_name and detected_object["confidence"] >= 0.8:
                    ...
                    for _id in loaded_camera:
                        ...
                        var = glb.loaded_camera_ids[source_details["source_id"]]["extra"][_id]["roi"]["roi_type"]
                        if var == "inside":
                            """
                                What we need to do in this is we are going to raise the alert of intrusion attempted.
                            """
